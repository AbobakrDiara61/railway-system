...
import { Train, User, CreditCard, CheckCircle, ChevronRight, ArrowLeft,
...
const JOURNEY = {
  train: 'Golden Horus Express',
  from: 'Cairo', to: 'Luxor',
  date: 'Oct 26, 2024',
  seat: 'First Class, Coach A, Seat 14 (Window)',
  baseFare: 3800, serviceFee: 300, taxes: 150,
};
...
export function BookingPage() {
  ...
  const { id }     = useParams();
  const [step, setStep] = useState(3); // show payment step to match reference
  ...
  const total = JOURNEY.baseFare + JOURNEY.serviceFee + JOURNEY.taxes;

  const handleConfirm = async () => {
    setLoading(true);
    try {
      await createBooking({ journeyId: id, paymentMethod: payMethod });
      setStep(4);
    } catch {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8">
        ...
        {step === 4 ? (
          ...
          <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
            ...
            <p className="text-slate-400 mb-8">Your ticket has been booked. Check your email for the confirmation.</p>
            <div className="glass-sm p-4 rounded-xl mb-8 text-left space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-500">Train</span><span className="text-white font-medium">{JOURNEY.train}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-500">Route</span><span className="text-white font-medium">{JOURNEY.from} → {JOURNEY.to}</span></div>
            ...
            <div className="flex gap-3">
              <button onClick={() => navigate('/bookings')} className="btn-navy flex-1 py-3 text-sm rounded-xl font-semibold">View My Trips</button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <AnimatePresence mode="wait">
                          ...
                          <div className="md:col-span-2 space-y-2">
                            ...
                            <div className="relative">
                              <input type="text" placeholder="0000 0000 0000 0000"
                                ...
                                maxLength={19}
                                className="input-glass font-mono text-lg tracking-widest"
                                style={{ borderColor: card.number ? 'rgba(0,229,255,0.4)' : undefined }}
                              />
                            </div>
                          </div>
                ...
                {step === 1 && (
                  <motion.div key="route"
                    ...
                    className="glass p-6 rounded-2xl border border-[rgba(0,229,255,0.12)] text-slate-300 text-sm">
                    <p>Select your route from the search results page.</p>
                  </motion.div>
                )}
            ...
            <div className="glass rounded-2xl p-5 border border-[rgba(245,158,11,0.15)] h-fit">
              ...
              <div className="border-t border-[rgba(255,255,255,0.06)] pt-4 mb-5 space-y-2">
                {[
                  ['Base Fare',    JOURNEY.baseFare],
                  ['Service Fee',  JOURNEY.serviceFee],
                  ['Taxes',        JOURNEY.taxes],
                ].map(([k,v]) => (
                  <div key={k} className="flex justify-between text-sm">
                    <span className="text-slate-500">{k}</span>
                    <span className="text-slate-300">{v.toLocaleString()} EGP</span>
                  </div>
                ))}
              </div>
